...
if (shape == null || shape == "rect") {
    ...
    for (var r = 0; r < rows; r++) {
        ...
        for (var c = 0; c < cols; c++) {
            ...
            if (
                (ca * ca) / ((cols / 4) * cols) + (ra * ra) / ((rows / 4) * rows) <=
                1 &&
                (ca * ca) / ((cols / 4) * cols) + (ra * ra) / ((rows / 4) * rows) >=
                0.1
            ) {
...
window.addEventListener("load", function () {
    ...
    document
        ...
        .setAttribute("onclick", "window.location.href='index.html" + extra + "'");
    ...
    for (var r = 0; r < rows; r++) {
        for (var c = 0; c < cols; c++) {
            if (grid[r][c] == "u") {
                ...
                button.style.padding = "0";
                button.style.left = c * 35 + 8 + "px";
                button.style.top = r * 35 + 48 + "px";
    ...
    for (var ra = 0; ra < rows; ra++) {
        for (var ca = 0; ca < cols; ca++) {
            ...
            if (ra == 0 || visu_grid[ra - 1][ca] == "-") {
                document.getElementById("but_" + ra + "m" + ca).style.borderTop =
                    "2px solid #095210";
    ...
    for (var ra = 0; ra < rows; ra++) {
        for (var ca = 0; ca < cols; ca++) {
            ...
            if (ca == 0 || visu_grid[ra][ca - 1] == "-") {
                document.getElementById("but_" + ra + "m" + ca).style.borderLeft =
                    "2px solid #095210";
    ...
    for (var ra = 0; ra < rows; ra++) {
        for (var ca = 0; ca < cols; ca++) {
            ...
            if (ra == rows - 1 || visu_grid[ra + 1][ca] == "-") {
                document.getElementById("but_" + ra + "m" + ca).style.borderBottom =
                    "2px solid #095210";
            } else if (visu_grid[ra + 1][ca] != visu_grid[ra][ca]) {
                document.getElementById("but_" + ra + "m" + ca).style.borderBottom =
                    "1px solid #095210";
    ...
    for (var ra = 0; ra < rows; ra++) {
        for (var ca = 0; ca < cols; ca++) {
            ...
            if (ca == cols - 1 || visu_grid[ra][ca + 1] == "-") {
                document.getElementById("but_" + ra + "m" + ca).style.borderRight =
                    "2px solid #095210";
            } else if (visu_grid[ra][ca + 1] != visu_grid[ra][ca]) {
                document.getElementById("but_" + ra + "m" + ca).style.borderRight =
                    "1px solid #095210";
            ...
            for (var co = 0; co < cols; co++) {
                if (grid[ro][co] == "ub") {
                    ...
                    if (shape == "donut") {
                        ...
                        document
                            .getElementById("but_" + ro + "m" + co)
                            .classList.remove("bute");
                        document
                            .getElementById("but_" + ro + "m" + co)
                            .classList.remove("buto");
                    }
...
function check_win() {
    ...
    if (!fail) {
        ...
        console.log(start_time, end_time, end_time - start_time);
    }
}
var counting;
function clickHandler(r, c, e) {
    if (!playing) {
        return;
    }
    if (!gridmade) {
        ...
        counting = setInterval(() => {
            if (playing) {
                document.getElementById("time").textContent =
                    "" + Math.floor((new Date().getTime() - start_time) / 10) / 100;
            } else {
                document.getElementById("time").textContent =
                    "" + Math.floor((end_time - start_time) / 10) / 100;
            }
        }, 10);
        var min_cnt = 0;
